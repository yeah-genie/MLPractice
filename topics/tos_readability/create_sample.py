...
# 샘플 텍스트 조각들
CLAUSES = {
    "header": "이용약관\n\n제1조 (목적)\n본 약관은 {app} 서비스 이용과 관련하여 {app}와 이용자 간의 권리·의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.\n\n",
    "long_sentence": "회사는 이용자가 개인정보처리방침 및 운영정책을 충분히 이해하고 이에 동의한 후 서비스를 이용하도록 함으로써 이용자의 권익을 보호하고자 하며, 이용자는 본 이용계약에 따라 서비스를 이용함에 있어 관련 법령의 규정을 준수하여야 하고, 이용자가 이를 위반할 경우 귀책사유에 따라 손해배상 책임이 발생할 수 있으며, 이 경우 회사는 면책되지 아니합니다.\n",
    "very_long_sentence": "본 서비스를 이용함으로써 귀하는 (1) 귀하가 서비스 이용 자격이 있는 연령임을 확인하고, (2) 귀하가 개인으로서 서비스를 이용하는 경우 귀하 개인을 대리하여 계약을 체결함을 확인하며, (3) 귀하가 사업체 또는 기타 법인을 대리하여 서비스를 이용하는 경우 해당 법인을 대리하여 계약을 체결할 권한이 있음을 확인하고, (4) 귀하는 이용계약에 대한 귀하의 동의를 통해 회사와 법적 구속력 있는 계약을 체결하며 이에 동의하는 것으로 간주하고, 이를 위반할 경우 관할법원에 소송이 제기될 수 있으며, 준거법은 대한민국 법률로 함을 이해하고 동의합니다.\n",
    ...
    "friendly": "궁금한 점이 있으시면 회원님께서 언제든지 고객센터를 통해 편리하게 문의하실 수 있습니다. 더 쉽고 빠르게 도움을 드릴 수 있도록 알려드립니다.\n",
    ...
    "translation_awkward": "귀하는 당사의 서비스를 이용함으로써 이용계약에 포함하되 이에 한정하지 않는 모든 조건에 동의하는 것으로 간주되며, 당사는 귀하에게 개인적, 비독점적, 양도 불가능한, 취소불가 라이선스를 부여하는 바이며, 기재된 바에 따라 귀하는 서비스를 이용할 수 있는 법적 구속력 있는 권리를 갖습니다.\n",
    "article": "제{n}조 ({title})\n",
}
...
ARTICLE_TITLES = [
    ...
    "광고 및 프로모션", "데이터 보관 정책", "서비스 이전", "기술적 조치", "최종 조항",
]  # 25개 — size="large" article_count=25 와 일치
...
def build_text(app: str, params: dict) -> str:
    ...
    for i, title in enumerate(ARTICLE_TITLES[:article_count], start=2):
        ...
        if sentence_style == "very_long":
            ...
            text += CLAUSES["medium_sentence"] * 2
            text += CLAUSES["friendly"]  # 구어체 앱은 친절한 표현 삽입
        else:  # short
            text += CLAUSES["short_sentence"] * 2
            text += CLAUSES["friendly"] * 2  # 당근마켓 스타일: 구어체 비중 높음
